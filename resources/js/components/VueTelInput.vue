<template>
    <div :class="['vue-tel-input', wrapperClasses, { disabled: disabled }]">
        <input
            ...
            :name="name"
            :class="[inputClasses, { 'is-invalid': errors.length }]"
            :id="inputId"
            :maxlength="maxLen"
    ...
    computed: {
        parsedPlaceholder() {
            ...
            if (this.dynamicPlaceholder) {
                ...
                return PhoneNumber.getExample(this.activeCountry.iso2, 'mobile').getNumber(mode).replace(`+${this.activeCountry.dialCode} `, '');
            }
    ...
    mounted() {
        this.initializeCountry()
            .then(() => {
                ...
                if (this.replaceInit) {
                    this.phone = this.phone.replace(`+${this.activeCountry.dialCode}`, '')
                }
                this.$emit('validate', this.phoneObject);
                this.$emit('onValidate', this.phoneObject); // Deprecated
            })
