<x-layout :title="trans('feedback.plural')" :breadcrumbs="['dashboard.feedback.index']">
    ...
    @component('dashboard::components.table-box')
        ...
        <thead>
            <tr>
                <th colspan="100">
                    <x-check-all-delete type="{{ \App\Models\Feedback::class }}" :resource="trans('feedback.plural')"></x-check-all-delete>
        ...
        <tbody>
            @forelse($feedback as $message)
                <tr class="{{ $message->read() ? 'tw-bg-gray-300' : 'font-weight-bold tw-bg-gray-100' }}">
                    ...
                    <td>
                        <a href="{{ route('dashboard.feedback.show', $message) }}" class="text-decoration-none text-ellipsis">
                            {{ $message->name }}
                        </a>
                    </td>
                    <td>{{ $message->phone }}</td>
